...
| BEA | Backfilling | PCE and Core PCE price indexes from NIPA | Free API key | Connector, live metadata-verified NIPA mappings, raw archive, normalization, and DB upsert path are tested. Live backfill verified on 2026-06-29 for NIPA `T20804` lines 1 and 25 with 874 observations in PostgreSQL. |
| EIA | Backfilling | Crude inventories | Free API key | Connector, raw archive, normalization, and DB upsert path are tested. Live backfill verified on 2026-06-29 for `PET.WCRSTUS1.W` with 2,282 observations in PostgreSQL. |
